...
make_internal_ds <- function() {
  ...
  calibration_data <- calibration$data %>%
    ...
    mutate(
      ecdf = cumsum(count) / calibration$S,
  ...
  stan_model_code <- rstan::stanc_builder(file.path("inst", "stan", "blrm_exnex.stan"), file.path("inst", "stan"), obfuscate_model_name = FALSE)$model_code
